...
local function fadeToggleActive(self, ply)
	if noKeyboard:GetBool() and not numpad.FromButton() then
		ply:ChatPrint("Вы не можете использовать FD с клавы на этом сервере")
		ply:ChatPrint("Попробуйте использовать кнопку или кейпад")
		return
	end
...
function TOOL:LeftClick(tr)
	...
	if self:GetClientNumber("useundo") == 1 then
